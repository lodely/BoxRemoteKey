...
public class AppPackagesHelper {
    ...
    public static void installPackage(final File apkFile, final Context context){
        try {
            ...
            intent.setDataAndType(getFileUri(context, apkFile), "application/vnd.android.package-archive");
    ...
    public static byte[] getAppIcon(String packageName, Context context){
        ...
        if(applicationInfo == null) return getDefaultAppIcon(context);
        try {
            android.graphics.drawable.Drawable drawable = applicationInfo.loadIcon(context.getPackageManager());
            if (drawable == null) return getDefaultAppIcon(context);
            BitmapDrawable bitmap = (BitmapDrawable)drawable;
            if (bitmap.getBitmap() == null) return getDefaultAppIcon(context);
            ByteArrayOutputStream data = new ByteArrayOutputStream();
            bitmap.getBitmap().compress(Bitmap.CompressFormat.PNG, 100, data);
            return data.toByteArray();
        } catch (Exception e) {
            Log.e(IMEService.TAG, "Failed to load icon for " + packageName, e);
            return getDefaultAppIcon(context);
        }
    }

    private static byte[] defaultIconCache = null;

    private static byte[] getDefaultAppIcon(Context context) {
        if (defaultIconCache != null) return defaultIconCache;
        try {
            java.io.InputStream is = context.getResources().openRawResource(R.drawable.default_app_icon);
            ByteArrayOutputStream data = new ByteArrayOutputStream();
            ...
            is.close();
            defaultIconCache = data.toByteArray();
